...
public class NumberPicker extends LinearLayout {
     ...
     */
    private final int mSelectorWheelItemCount;
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final int layoutResId = attributesArray.getResourceId(
                R.styleable.NumberPicker_internalLayout, R.layout.number_picker_material);
        ...
        mWrapSelectorWheelPreferred = attributesArray.getBoolean(
                R.styleable.NumberPicker_wrapSelectorWheel, true);

        mSelectorWheelItemCount = attributesArray.getInt(
                R.styleable.NumberPicker_selectorWheelItemCount, 3);
        if (mSelectorWheelItemCount < 1) {
            throw new IllegalArgumentException("selectorWheelItemCount must > 0");
        }
        mSelectorIndices = new int[mSelectorWheelItemCount];

        mTextSize = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_textSize, 40);

        ColorStateList textColor = attributesArray.getColorStateList(R.styleable.NumberPicker_textColor);

        if (textColor != null) {
            mTextColor = textColor;
        }

        mInputEnabled = attributesArray.getBoolean(
                R.styleable.NumberPicker_inputEnabled, true);
        ...
        if (mInputEnabled) mInputText.setVisibility(VISIBLE);
        else mInputText.setVisibility(INVISIBLE);
     ...
     */
    private int getSelectorMiddleItemIndex() {
        return mSelectorWheelItemCount / 2;
    }
    ...
    public void scrollBy(int x, int y) {
        ...
        if (!mWrapSelectorWheel && y > 0
                && selectorIndices[getSelectorMiddleItemIndex()] <= mMinValue) {
            mCurrentScrollOffset = mInitialScrollOffset;
            return;
        }
        if (!mWrapSelectorWheel && y < 0
                && selectorIndices[getSelectorMiddleItemIndex()] >= mMaxValue) {
            mCurrentScrollOffset = mInitialScrollOffset;
            return;
        }
        ...
        while (mCurrentScrollOffset - mInitialScrollOffset > mSelectorTextGapHeight) {
            ...
            setValueInternal(selectorIndices[getSelectorMiddleItemIndex()], true);
            if (!mWrapSelectorWheel && selectorIndices[getSelectorMiddleItemIndex()] <= mMinValue) {
                mCurrentScrollOffset = mInitialScrollOffset;
            }
        }
        while (mCurrentScrollOffset - mInitialScrollOffset < -mSelectorTextGapHeight) {
            ...
            setValueInternal(selectorIndices[getSelectorMiddleItemIndex()], true);
            if (!mWrapSelectorWheel && selectorIndices[getSelectorMiddleItemIndex()] >= mMaxValue) {
                mCurrentScrollOffset = mInitialScrollOffset;
            }
        }
    ...
    public boolean performClick() {
        if (!super.performClick() && mInputEnabled) {
            showSoftInput();
        }
        return true;
    }
    ...
    public boolean performLongClick() {
        if (!super.performLongClick() && mInputEnabled) {
            showSoftInput();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        ColorStateList colors = mInputText.getTextColors();
        int color = colors.getColorForState(getDrawableState(), Color.WHITE);
        ...
        for (int i = 0; i < selectorIndices.length; i++) {
            ...
            // with the new one.
            if ((showSelectorWheel && i != getSelectorMiddleItemIndex()) ||
                    (i == getSelectorMiddleItemIndex() && mInputText.getVisibility() != VISIBLE)) {
    ...
    private void initializeSelectorWheelIndices() {
        ...
        for (int i = 0; i < mSelectorIndices.length; i++) {
            int selectorIndex = current + (i - getSelectorMiddleItemIndex());
    ...
    private void initializeSelectorWheel() {
        initializeSelectorWheelIndices();
        int[] selectorIndices = mSelectorIndices;
        float totalTextHeight = selectorIndices.length * mTextSize;
        ...
        mInitialScrollOffset = editTextTextPosition
                - (mSelectorElementHeight * getSelectorMiddleItemIndex());
        mCurrentScrollOffset = mInitialScrollOffset;
        updateInputTextView();
    }
    ...
    private void initializeFadingEdges() {
        ...
        setFadingEdgeLength((int) ((getBottom() - getTop() - mTextSize) / 2));
    }
